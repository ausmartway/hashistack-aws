...
resource "aws_autoscaling_group" "hashistack" {
  ...
  max_size             = var.numbers != -1 ? var.numbers : length(var.subnet_ids)
  min_size             = var.numbers != -1 ? var.numbers : length(var.subnet_ids)
  desired_capacity     = var.numbers != -1 ? var.numbers : length(var.subnet_ids)
  default_cooldown     = 30
  force_delete         = true
